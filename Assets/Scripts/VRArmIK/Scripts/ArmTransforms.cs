...
namespace VRArmIK
{
	public class ArmTransforms : MonoBehaviour
	{
		...
		void Start()
		{
			networkPoseManager.onCalibrate += updateArmLengths;
			updateArmLengths();
		...
		{
			var shoulderWidth = networkPoseManager.playerWidthShoulders;
			var _armLength = (networkPoseManager.playerWidthWrist - shoulderWidth) / 2f;
